...
class TestQRToken:
    def test_generate_qr_token(self, client, created_session):
    ...
    def test_generate_qr_token_session_not_found(self, client):
        response = client.get("/attendance/qr/token/9999")
        assert response.status_code == 404


class TestQRMark:
    def test_mark_qr_attendance_success(self, client, created_member, created_session):
        # Generate QR token
    ...
    def test_mark_qr_attendance_expired_token(self, client, created_member, created_session):
    ...
    def test_mark_qr_attendance_invalid_auth(self, client, created_session):
        qr_resp = client.get(f"/attendance/qr/token/{created_session['id']}")
        qr_token = qr_resp.json()["token"]
    ...
    def test_mark_qr_attendance_wrong_session(self, client, created_member, created_session):
    ...
    def test_mark_qr_missing_auth_header(self, client, created_session):
    ...
    def test_mark_qr_invalid_token_type(self, client, created_member, created_session):
        """A regular auth token (not QR) should be rejected as QR token."""
        auth_token = create_access_token(data={"sub": created_member["email"]})
